...
class Controller_lib:
    def __init__(self):
        ...
        self._uav_mass = 1.0  # UAV mass
        self._inertia_matrix = np.array([1.0, 1.0, 1.0])  # Inertia matrix
        self._gravity = 9.81  # Gravity

        # Initialize control gains with placeholder values
        self.position_gain = np.array([1.0, 1.0, 1.0])
        self.velocity_gain = np.array([1.0, 1.0, 1.0])
        self.attitude_gain = np.array([1.0, 1.0, 1.0])
        self.angular_rate_gain = np.array([1.0, 1.0, 1.0])

        # Initialize state and reference values with zeros
        self.position_W = np.zeros(3)
        self.velocity_W = np.zeros(3)
        self.orientation_B_W_temp = np.zeros(4)
        self.R_B_W = np.eye(3)
    ...
    def set_odometry(self, position_W, velocity_W, orientation_B_W, angular_velocity_B):
        ...
        self.orientation_B_W_temp = np.array([orientation_B_W[1], orientation_B_W[2], orientation_B_W[3], orientation_B_W[0]])  #Quaternion reordering (w, x, y, z)
    ...
    def set_trajectory_point(self, position_W, orientation_W):
        ...
        self.r_velocity_W = np.zeros(3)
        self.r_acceleration_W = np.zeros(3)
        self.orientation_W_temp = np.array([orientation_W[1], orientation_W[2], orientation_W[3], orientation_W[0]])  #Quaternion reordering (w, x, y, z)
        self.r_R_B_W = R.from_quat(orientation_W).as_matrix()
        self.r_yaw = R.from_quat(orientation_W).as_euler('xyz', degrees=False)[2]
        self.r_yaw_rate = 0.0
    ...
    def calculate_controller_output(self):
        ...
        # Initialize controller torque thrust to have 4 elements
        ...
        I_a_d = -np.multiply(self.position_gain, e_p) - np.multiply(self.velocity_gain, e_v) + \
                self.uav_mass * self.gravity * np.array([0.0, 0.0, 1.0]) + self.uav_mass * self.r_acceleration_W
        ...
        e_omega = self.angular_velocity_B - self.R_B_W.T @ R_d_w @ omega_ref
        tau = -np.multiply(self.attitude_gain, e_R) - np.multiply(self.angular_rate_gain, e_omega) + \
              np.cross(self.angular_velocity_B, np.diag(self.inertia_matrix) @ self.angular_velocity_B)
...
# Example of setting up the controller and using it
controller = Controller()
controller.set_uav_parameters(1.0, np.array([0.1, 0.1, 0.2]), 9.81)
controller.set_control_gains(np.array([1.0, 1.0, 1.0]), np.array([1.0, 1.0, 1.0]), np.array([1.0, 1.0, 1.0]), np.array([1.0, 1.0, 1.0]))
controller.set_odometry(np.array([0.0, 0.0, 0.0]), np.array([0.0, 0.0, 0.0]), np.array([0.0, 0.0, 0.0, 1.0]), np.array([0.0, 0.0, 0.0]))    ##예시값
controller.set_trajectory_point(np.array([0.0, 0.0, 0.0]), np.array([0.0, 0.0, 0.0, 1.0]))                                                  ##예시값    

new_controller_torque_thrust, new_desired_quaternion = controller.calculate_controller_output()